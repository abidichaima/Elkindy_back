...
module.exports.getUsersByRole = async (req, res, next) => {
  ...
  // Define route to get lessons by user ID
  module.exports.getLessonByUser = async (req, res, next) => {
    try {
      const userId = req.params.userId;

      // Find lessons where the given user is either the teacher or one of the students
      const lessons = await Lesson.find({
          $or: [
              { teacher: userId },
              { students: { $in: [userId] } }
          ]
      }).populate('teacher students course'); // Populate referenced documents
